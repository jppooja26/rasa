...
class Custom_dataset(Dataset):
    def __init__(self, transform, image_root):
        self.transform = transform
        self.image_root = image_root
        self.imagelist = []
        for i in glob.glob(self.image_root + '*.png', recursive=True):
            print(i)
...
@torch.no_grad()
def inference(model, data_loader, tokenizer, device, config):
    # Text image matching
    model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Evaluation:'
    ...
    """
    #Added
    #text = "the man is wearing a black coat and black trousers he is wearing glasses he walked with his hands on each other"
    text = "the male is wearing red shirt"
    text_input = tokenizer(text, padding='max_length', truncation=True, max_length=config['max_words'], return_tensors="pt").to(device)
    text_output = model.text_encoder.bert(text_input.input_ids, attention_mask=text_input.attention_mask, mode='text')
    text_feat = text_output.last_hidden_state
    text_embed = F.normalize(model.text_proj(text_feat[:, 0, :]))
    ...
    batch_size = 32
    path_of_the_directory = 'D:\project\Rasa\dataset\RSTPReid'
    ...
    #    print(filename)
    #X_train = np.load(path_of_the_directory)
    #image_dataset = ImageDataset(ims=X_train)
    #data_loader = DataLoader(image_dataset, batch_size, shuffle=True, num_workers=3, pin_memory=True)
    
    print("Files and directories in a specified path:", path_of_the_directory)
    ...
    for indexes_list in indexes.cpu().numpy():
        for i in indexes_list:
            print("indexes = ", i)
            print("dataset.get_filepath(i) = ", dataset.get_filepath(i))
            with Image.open(dataset.get_filepath(i)) as im:
                im.save(path_of_the_directory + os.path.basename(dataset.get_filepath(i)))
    
    score_matrix_t2i = torch.full((len(texts), len(data_loader.dataset.image)), -100.0).to(device)
    print("score_matrix_t2i = ", score_matrix_t2i)
    # take the top-k candidates and calculate their ITM score sitm for ranking
    num_tasks = utils.get_world_size()
    rank = utils.get_rank()
    print("rank = ", rank)
...
def main(args, config):
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        if epoch >= config['eval_epoch'] or args.evaluate:
            #Added for inference
            score_test_t2i = inference(model_without_ddp, test_loader, tokenizer, device, config)
            break
            #End
